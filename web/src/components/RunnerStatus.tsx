...
export interface RunnerHealth {
  ...
  requests_cancelled?: number | null
  ...
  return (
    <section className="runner-status" aria-labelledby="runner-status-title">
      ...
      <div className="runner-status__grid">
        ...
        <article className="runner-status__card"><h3>Requests</h3><p className="runner-status__scope">Since runner startup</p><dl className="runner-status__stats"><div><dt>Completed</dt><dd>{numberValue(health?.requests_completed)}</dd></div><div><dt>Failed</dt><dd>{numberValue(health?.requests_failed)}</dd></div><div><dt>Cancelled</dt><dd>{numberValue(health?.requests_cancelled)}</dd></div><div><dt>Last completion</dt><dd>{numberValue(health?.last_completion_tokens)}</dd></div></dl></article>
      </div>
